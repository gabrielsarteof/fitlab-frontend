...
import CriarPersonalModal from '@components/modals/CriarPersonalModal';
...
export default function PersonalPage() {
  ...
  const fetchPersonais = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/personaltrainers');
      setPersonais(data);
    } catch (err) {
      const msg = err.response?.data?.message || err.message || 'Erro ao buscar personal trainers';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async id => {
    if (!window.confirm('Confirma exclusão deste personal trainer?')) return;
    setLoading(true);
    try {
      await api.delete(`/personaltrainers/${id}`);
      toast.success('Personal trainer excluído com sucesso!');
      fetchPersonais();
    } catch (err) {
      const msg = err.response?.data?.message || err.message || 'Erro ao excluir personal trainer';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const filtrados = personais.filter(p =>
    ...
    p.certificacao.toLowerCase().includes(busca.toLowerCase())
  );

  const ordenados = [...filtrados].sort((a, b) => {
    if (sortKey === 'nome-asc') return a.nome.localeCompare(b.nome);
    if (sortKey === 'nome-desc') return b.nome.localeCompare(a.nome);
    if (sortKey === 'certificacao') return a.certificacao.localeCompare(b.certificacao);
    return 0;
  });
  ...
  return (
    <div className="bg-white py-4 px-3 px-md-4">
      ...
      <div className="mb-3 d-flex flex-column flex-md-row align-items-center justify-content-between">
        <div>
          <h3 className="mb-1 fw-bold">Personal Trainers</h3>
      ...
      <div className="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2 mb-4">
        ...
        <div className="dropdown ms-md-auto">
          ...
          <ul className="dropdown-menu dropdown-menu-end rounded-1 border-0 shadow">
            ...
            <li><button className="dropdown-item" onClick={() => setSortKey('certificacao')}>Certificação</button></li>
          </ul>
        </div>
      </div>
      ...
      <div className="d-none d-md-flex bg-light rounded-1 py-2 px-3 fw-bold">
        ...
        <div className="col-2">Certificação</div>
      ...
      <div className="mt-3 d-flex flex-column gap-3">
        ...
        {!loading && ordenados.map(p => (
          <div key={p.id} className="d-flex flex-column flex-md-row border border-light rounded-1 bg-white py-2 px-3">
            ...
            <div className="col-12 col-md-3 fw-bold mb-1 mb-md-0 text-truncate">{p.nome}</div>
            <div className="col-12 col-md-3 mb-1 mb-md-0 text-truncate">{p.email}</div>
            <div className="col-12 col-md-2 mb-1 mb-md-0 text-truncate">{p.telefone}</div>
            <div className="col-12 col-md-2 mb-1 mb-md-0 text-truncate">{p.certificacao}</div>
      ...
      <CriarPersonalModal
        ...
        onSaved={() => { setShowModal(false); fetchPersonais(); }}
      />
    </div>
  );
}
