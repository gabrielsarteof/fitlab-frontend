...
import CriarNutricionistaModal from '@components/modals/CriarNutricionistaModal';
...
export default function NutricionistasPage() {
  ...
  const fetchNutricionistas = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/nutricionistas');
      setNutricionistas(data);
    } catch (err) {
      const msg = err.response?.data?.message || err.message || 'Erro ao buscar nutricionistas';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async id => {
    if (!window.confirm('Confirma exclusão deste nutricionista?')) return;
    setLoading(true);
    try {
      ...
      const msg = err.response?.data?.message || err.message || 'Erro ao excluir nutricionista';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const ordenados = [...filtrados].sort((a, b) => {
    ...
    if (sortKey === 'especialidade') return a.especialidade.localeCompare(b.especialidade);
    return 0;
  });
  ...
  return (
    <div className="bg-white py-4 px-3 px-md-4">
      ...
      <div className="mb-3 d-flex flex-column flex-md-row align-items-center justify-content-between">
        <div>
          <h3 className="mb-1 fw-bold">Nutricionistas</h3>
      ...
      <div className="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2 mb-4">
        <SearchInput
          value={busca}
          onChange={setBusca}
          placeholder="Pesquisar nutricionista..."
        />
        <div className="dropdown ms-md-auto">
          ...
          <ul className="dropdown-menu dropdown-menu-end rounded-1 border-0 shadow">
            ...
            <li><button className="dropdown-item" onClick={() => setSortKey('especialidade')}>Especialidade</button></li>
          </ul>
        </div>
      </div>
      ...
      <div className="d-none d-md-flex bg-light rounded-1 py-2 px-3 fw-bold">
        ...
        <div className="col-2">Especialidade</div>
      ...
      <div className="mt-3 d-flex flex-column gap-3">
        ...
        {!loading && ordenados.map(n => (
          <div key={n.id} className="d-flex flex-column flex-md-row border border-light rounded-1 bg-white py-2 px-3">
            <div className="col-12 col-md-1 mb-1 mb-md-0">#{n.id}</div>
            <div className="col-12 col-md-3 fw-bold mb-1 mb-md-0 text-truncate">{n.nome}</div>
            <div className="col-12 col-md-3 mb-1 mb-md-0 text-truncate">{n.email}</div>
            <div className="col-12 col-md-2 mb-1 mb-md-0 text-truncate">{n.telefone}</div>
            <div className="col-12 col-md-2 mb-1 mb-md-0 text-truncate">{n.especialidade}</div>
            <div className="col-12 col-md-1 d-flex justify-content-start gap-2">
              <button className="btn action-btn border-0 py-1 px-2" onClick={() => { setEditId(n.id); setShowModal(true); }}>
                <i className="bi bi-pencil fs-6"></i>
              </button>
              <button className="btn bg-danger-subtle border-0 py-1 px-2" onClick={() => handleDelete(n.id)}>
      ...
      <CriarNutricionistaModal
        show={showModal}
        nutricionistaId={editId}
        onClose={() => setShowModal(false)}
        onSaved={() => { setShowModal(false); fetchNutricionistas(); }}
      />
    </div>
  );
}
